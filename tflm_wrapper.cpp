...
// Arena (Mémoria RAM reservada para o modelo pensar)
// 120KB é um bom tamanho para o RP2040
...
extern "C" int tflm_init(void) {
    ...
    // --- CORREÇÃO AQUI ---
    // Aumentei de <8> para <10> para garantir espaço
    static tflite::MicroMutableOpResolver<10> resolver;

    resolver.AddConv2D();
    resolver.AddMaxPool2D();       // <--- ADICIONADO: Resolve o erro "Didn't find op MAX_POOL_2D"
    resolver.AddMean();            // (GlobalAveragePooling2D) - Pode manter
    resolver.AddFullyConnected();  // (Dense Layer)
    resolver.AddSoftmax();         // (Output Layer)
    resolver.AddReshape();         // (Flatten)
    resolver.AddQuantize();        // Necessário para modelos INT8
    resolver.AddDequantize();      // Necessário para modelos INT8
